...
//       // 5. Initialize empty profile in localStorage
...
// export default Signup;
    ...
    <section className="py-20 px-6 bg-white dark:bg-gray-900">
      <div className="max-w-7xl mx-auto">
        ...
        >
          <h2 className="text-4xl font-light text-gray-900 dark:text-white mb-3 font-Quicksand">
            Featured <span className="font-semibold">Masterpieces</span>
          </h2>
          <p className="text-lg text-gray-600 dark:text-gray-300 max-w-xl mx-auto font-Roboto">
            ...
            >
              ...
              <div className="relative h-80 overflow-hidden">
                <motion.img
                  ...
                  initial={{ scale: 1 }}
                  animate={{ scale: hoveredImage === index ? 1.05 : 1 }}
              ...
              <div className="absolute bottom-0 left-0 right-0 p-4 flex flex-col">
                <h3 className="text-white font-semibold text-xl mb-1 drop-shadow">{image.title}</h3>
                <p className="text-white/80 text-sm mb-3">{image.category}</p>

                <motion.div
                  className="flex gap-2"
                  initial="hidden"
                  animate={hoveredImage === index ? "visible" : "hidden"}
                  variants={{
                    hidden: { opacity: 0, y: 10 },
                    visible: { opacity: 1, y: 0, transition: { staggerChildren: 0.1 } }
                  }}
                >
                  {[FiHeart, FiShare2, FiDownload].map((Icon, idx) => (
                    <motion.button
                      key={idx}
        ...
        >    
        <button className="inline-flex items-center px-8 py-4 bg-gray-900 dark:bg-white text-white dark:text-gray-900 rounded-lg font-semibold hover:bg-gray-800 dark:hover:bg-gray-200 transition-all shadow-lg hover:shadow-xl"
        >Load More</button>      

...
import BackImg from './Sub_components_images/water-colour.jpg';
import { Link } from 'react-router-dom';
import { FaHome, FaUser, FaInfoCircle, FaPalette, FaSearch,FaRegComment, FaArrowLeft, FaArrowRight,FaRegEye } from 'react-icons/fa';
...
function Water_color() {
  ...
  const cards = infoCardsData.watercolor;
  ...
  useEffect(() => {
    const fetchWaterImages = async () => {
      try {
        ...
        const response = await databases.listDocuments(
          ...
          [      
            Query.equal( 'medium', 'Watercolor Painting')] // Query for landscape tag
        );
    ...
    //Get URLs for each file
      ...
      setWaterImages(imagesWithUrls.filter(image => image !== null));
      } catch (err) {
  ...
  const downloadImage = (url, title) => {
    ...
    link.href = url;
    link.download = title || 'Portrait-image';
    link.click();
  };
  ...
  const prevImage = () => setLightbox((prev) => ({
    ...prev,
    index: prev.index > 0 ? prev.index - 1 : waterImages.length - 1,
  }));
  const nextImage = () => setLightbox((prev) => ({
    ...prev,
    index: prev.index < waterImages.length - 1 ? prev.index + 1 : 0,
  }));
  ...
  useEffect(() => {
    const fetchImages = async () => {
      ...
      setFilteredImages(fetchWaterImages);
    };
    fetchImages();
  }, []);
    ...
    <div className='h-screen w-screen overflow-x-hidden bg-gray-100 dark:bg-gray-900 font-Playfair'>
        ...
        <header className='fixed top-0 h-[80px] w-full bg-white/20 backdrop-blur-sm flex items-center justify-between px-4 md:px-6 shadow-md z-50'>
          ...
          <div className='flex items-center gap-x-2 sm:gap-x-4'>
            {/* Search Bar */}
             <div>
             <SearchBar 
             allImages={waterImages} 
             onFilter={setFilteredImages} 
             placeholder="Search Water colour..." 
             />
             </div>
            {/* Desktop Navigation */}
      ...
      <section ref={contentRef} className='w-[85%] py-12 bg-gray-100 dark:bg-gray-900  mx-auto'>
        <div className='max-w-7xl mx-auto text-center mb-8'>
             <h2 className='text-2xl md:text-3xl font-bold font-Playfair text-gray-800 dark:text-gray-300 mb-4'>Whispers of Light – The Fluid Grace of Watercolor</h2>
      ...
      <AnimatePresence>
        {lightbox.open && (
          ...
          >
            ...
            >
              <img
                src={waterImages[lightbox.index].url}
                alt={waterImages[lightbox.index].title || 'Landscape image'}

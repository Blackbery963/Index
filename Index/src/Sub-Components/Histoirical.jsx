...
import BackImg from './Sub_components_images/historical.jpg';
...
function Historical() {
  ...
  const cards = infoCardsData.historical;
  ...
  //         [      
  //           Query.equal( 'tag', 'Historical')] // Query for landscape tag
  //       );
  ...
  //     setHistoricalImages(imagesWithUrls.filter(image => image !== null));
  //     } catch (err) {
  ...
  useEffect(() => {
    const fetchHistoricalImages = async () => {
      try {
        ...
        setHistoricalImages(validImages);
        ...
        setError(err.message || 'Failed to load Historical images');
      } finally {
        setLoading(false);
      }
    };
  ...
  const downloadImage = (url, title) => {
    ...
    link.href = url;
    link.download = title || 'Historical-image';
    link.click();
  };
  ...
  const prevImage = () => setLightbox((prev) => ({
    ...prev,
    index: prev.index > 0 ? prev.index - 1 : historicalImages.length - 1,
  }));
  const nextImage = () => setLightbox((prev) => ({
    ...prev,
    index: prev.index < historicalImages.length - 1 ? prev.index + 1 : 0,
  }));
  ...
  useEffect(() => {
    const fetchImages = async () => {
      ...
      setFilteredImages(fetchHistoricalImages);
    };
    fetchImages();
  }, []);
    ...
    <div className='h-screen w-screen overflow-x-hidden bg-gray-100 dark:bg-gray-900 font-Playfair'>
      ...
      <section ref={contentRef} className='w-[85%] py-12 bg-gray-100 dark:bg-gray-900  mx-auto'>
        <div className='max-w-7xl mx-auto text-center mb-8'>
             <h2 className='text-2xl md:text-3xl font-bold font-Playfair text-gray-800 dark:text-gray-200 mb-4'>The Timeless Legacy of History Captured in Art</h2>
      ...
      <AnimatePresence>
        {lightbox.open && (
          ...
          >
            ...
            >
              <img
                src={historicalImages[lightbox.index].url}
                alt={historicalImages[lightbox.index].title || 'Landscape image'}

...
function Modern() {
  ...
  const [modernImages, setModernImages] = useState([]);
  ...
  const downloadImage = (url, title) => {
    ...
    link.href = url;
    link.download = title || 'Modern-image';
    link.click();
  };
  ...
  const prevImage = () => setLightbox((prev) => ({
    ...prev,
    index: prev.index > 0 ? prev.index - 1 : modernImages.length - 1,
  }));
  const nextImage = () => setLightbox((prev) => ({
    ...prev,
    index: prev.index < modernImages.length - 1 ? prev.index + 1 : 0,
  }));
    ...
    <div className='h-screen w-screen overflow-x-hidden bg-gray-100 dark:bg-gray-900 font-Playfair'>
      ...
      <div className='h-[80vh] w-full bg-center bg-cover' style={{ backgroundImage: `url(${BackImg})` }}>
        ...
        <Link to={'/'}>
          ...
          <div className='flex items-center gap-x-2 sm:gap-x-4'>
            {/* Search Bar */}
             <div>
             <SearchBar 
             allImages={modernImages} 
             onFilter={setFilteredImages} 
             placeholder="Search Modern Art..." 
             />
             </div>
            {/* Desktop Navigation */}
        ...
        <main className='flex flex-col items-center justify-center h-full px-4 text-center'>
          <h1 className='font-Tapestary text-[30px] md:text-[50px] text-yellow-300 drop-shadow-lg animate-fade-in'>Redefining Art</h1>
          <h5 className='font-Carattere text-[20px] md:text-[28px] text-black mt-4 drop-shadow-md animate-fade-in delay-200'>
          Discover a realm where tradition fades and innovation leads, capturing bold perspectives through modern creativity.
          </h5>
  ...
  <h2 className='text-2xl md:text-3xl font-bold font-Quicksand text-gray-800 dark:text-gray-200 mb-4'>Step into the Modern Art Movement</h2>
  <p className='text-base md:text-lg text-gray-600 dark:text-gray-300'>
    Bold ideas and daring visuals collide in modern art. Experience the freedom of expression that breaks all traditional rules.
  </p>
</div>
      ...
      <AnimatePresence>
        {lightbox.open && (
          ...
          >
            ...
            >
              <img
                src={modernImages[lightbox.index].url}
                alt={modernImages[lightbox.index].title || 'Landscape image'}

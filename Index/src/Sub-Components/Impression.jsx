...
import BackImg from './Sub_components_images/impression.jpg';
...
function Impression() {
  ...
  const cards = infoCardsData.impressionism;
  ...
  // useEffect(() => {
  //   const fetchImpressionImages = async () => {
  //     try {
  //       setLoading(true);
  ...
  //         [      
  //           Query.equal( 'tag', 'Impression')] // Query for landscape tag
  //       );
  ...
  //     setImpressionImages(imagesWithUrls.filter(image => image !== null));
  //     } catch (err) {
  ...
  useEffect(() => {
    const fetchImpressionImages = async () => {
      try {
        ...
        setImpressionImages(validImages);
        ...
        setError(err.message || 'Failed to load Impressionism images');
      } finally {
        setLoading(false);
      }
    };
  ...
  const prevImage = () => setLightbox((prev) => ({
    ...prev,
    index: prev.index > 0 ? prev.index - 1 : impressionImages.length - 1,
  }));
  const nextImage = () => setLightbox((prev) => ({
    ...prev,
    index: prev.index < impressionImages.length - 1 ? prev.index + 1 : 0,
  }));
  ...
  useEffect(() => {
    const fetchImages = async () => {
      ...
      setFilteredImages(fetchImpressionImages);
    };
    fetchImages();
  }, []);
  ...
  return (
    <div className='h-screen w-screen overflow-x-hidden bg-gray-100 dark:bg-gray-900 font-Playfair'>
      ...
      <div className='h-[80vh] w-full bg-center bg-cover' style={{ backgroundImage: `url(${BackImg})` }}>
        ...
        <h1 className='font-Tapestary text-[30px] md:text-[50px] text-orange-600 drop-shadow-lg animate-fade-in'>Moments in Light</h1>
<h5 className='font-Carattere text-[20px] md:text-[28px] text-yellow-200 mt-4 drop-shadow-md animate-fade-in delay-200'>
  Step into sunlit impressions and fleeting glances, where brushstrokes dance with color to capture life's essence.
</h5>
      ...
      <AnimatePresence>
        {lightbox.open && (
          ...
          >
            ...
            >
              <img
                src={impressionImages[lightbox.index].url}
                alt={impressionImages[lightbox.index].title || 'Landscape image'}

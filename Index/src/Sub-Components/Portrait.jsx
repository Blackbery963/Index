...
function Portrait() {
  ...
  const prevImage = () => setLightbox((prev) => ({
    ...prev,
    index: prev.index > 0 ? prev.index - 1 : portraitImages.length - 1,
  }));
  const nextImage = () => setLightbox((prev) => ({
    ...prev,
    index: prev.index < portraitImages.length - 1 ? prev.index + 1 : 0,
  }));
  ...
  useEffect(() => {
    const fetchImages = async () => {
      ...
      setFilteredImages(fetchPortraitImages);
    };
    fetchImages();
  }, []);
    ...
    <div className='h-screen w-screen overflow-x-hidden bg-gray-100 dark:bg-gray-900 font-Playfair'>
      ...
      <div className='h-[80vh] w-full bg-center bg-cover' style={{ backgroundImage: `url(${BackImg})` }}>
        ...
        <main className='flex flex-col items-center justify-center h-full px-4 text-center'>
          <h1 className='font-Tapestary text-[30px] md:text-[50px] text-red-600 drop-shadow-lg animate-fade-in'>Faces of Time</h1>
          <h5 className='font-Carattere text-[20px] md:text-[28px] text-yellow-300 mt-4 drop-shadow-md animate-fade-in delay-200'>
           Journey into the soul of humanity through portraits that echo emotion, heritage, and the stories behind every gaze.
          </h5>
  ...
  <h2 className='text-2xl md:text-3xl font-bold font-Quicksand text-gray-800 dark:text-gray-200 mb-4'>Explore the Art of Portraiture</h2>
  <p className='text-base md:text-lg text-gray-600 dark:text-gray-300'>
    Discover powerful portraits that reveal character, emotion, and human connection. Each brushstroke captures the essence of a soul.
  </p>
</div>
      ...
      <AnimatePresence>
        {lightbox.open && (
          ...
          >
            ...
            >
              <img
                src={portraitImages[lightbox.index].url}
                alt={portraitImages[lightbox.index].title || 'Landscape image'}

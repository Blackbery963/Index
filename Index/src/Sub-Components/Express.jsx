...
import BackImg from './Sub_components_images/expressism.jpg';
...
function Express() {
  ...
  const [expressImages, setExpressImages] = useState([]);
  const [loading, setLoading] = useState(true);
  const [error, setError] = useState(null)
  const [profileImage, setProfileImage] = useState(null);
  const cards = infoCardsData.expressionism;
  ...
  // useEffect(() => {
  //   const fetchExpressImages = async () => {
  //     try {
  //       setLoading(true);
  ...
  //         [      
  //           Query.equal( 'tag', 'Expressionism')] // Query for landscape tag
  //       );
  ...
  //     setExpressImages(imagesWithUrls.filter(image => image !== null));
  //     } catch (err) {
  ...
  useEffect(() => {
    const fetchExpressImages = async () => {
      try {
        ...
        setError(err.message || 'Failed to load Expressionism images');
      } finally {
        setLoading(false);
      }
    };
  ...
  useEffect(() => {
    const fetchImages = async () => {
      ...
      setFilteredImages(fetchExpressmages);
    };
    fetchImages();
  }, []);
    ...
    <div className='h-screen w-screen overflow-x-hidden bg-gray-100 dark:bg-gray-900 font-Playfair'>
      ...
      <div className='h-[80vh] w-full bg-center bg-cover' style={{ backgroundImage: `url(${BackImg})` }}>
        ...
        <main className='flex flex-col items-center justify-center h-full px-4 text-center'>
          <h1 className='font-Tapestary text-[30px] md:text-[50px] text-gray-400 drop-shadow-lg animate-fade-in'>Feel Every Color</h1>
<h5 className='font-Carattere text-[20px] md:text-[28px] text-yellow-500 mt-4 drop-shadow-md animate-fade-in delay-200'>
  Step into raw emotion and intensity—where color and form explode in passion,revealing the artist’s inner world.
</h5>
  ...
  <h2 className='text-2xl md:text-3xl font-bold font-Quicksand text-gray-800 dark:text-gray-200 mb-4'>Feel the Pulse of Expressionism</h2>
  <p className='text-base md:text-lg text-gray-600 dark:text-gray-300'>
    Expressionist works ignite raw emotion. Explore art that speaks from the soul, bursting with energy, distortion, and color.
  </p>
</div>
      ...
      <AnimatePresence>
        {lightbox.open && (
          ...
          >
            ...
            >
              <img
                src={expressImages[lightbox.index].url}
                alt={expressImages[lightbox.index].title || 'Landscape image'}
              ...
              <div className="absolute bottom-4 left-0 right-0 text-center text-white font-Quicksand">
                ...
                <p className="text-sm">{lightbox.index + 1} / {expressImages.length}</p>
               <div className="absolute top-4 left-4">
                    <ArtworkViewTracker artworkId={expressImages[lightbox.index].$id} />
                  </div>
              </div>
            </motion.div>
          </motion.div>

...
import BackImg from './Sub_components_images/surealism.jpg';
...
function Surrealism() {
  ...
  const [surrealismImages, setSurrealismImages] = useState([]);
  const [loading, setLoading] = useState(true);
  const [error, setError] = useState(null)
  const [profileImage, setProfileImage] = useState(null);
  const cards = infoCardsData.surrealism;
...
useEffect(() => {
  const fetchSurrealismImages = async () => {
    try {
      setLoading(true);
      ...
      setSurrealismImages(validImages);
      ...
      setError(err.message || 'Failed to load Surrealism images');
    } finally {
      setLoading(false);
    }
  };

  fetchSurrealismImages();
}, []);
  ...
  const downloadImage = (url, title) => {
    ...
    link.href = url;
    link.download = title || 'Impression-image';
    link.click();
  };
  ...
  const prevImage = () => setLightbox((prev) => ({
    ...prev,
    index: prev.index > 0 ? prev.index - 1 : surrealismImages.length - 1,
  }));
  const nextImage = () => setLightbox((prev) => ({
    ...prev,
    index: prev.index < surrealismImages.length - 1 ? prev.index + 1 : 0,
  }));
  ...
  useEffect(() => {
    const fetchImages = async () => {
      ...
      setFilteredImages(fetchSurrealismImages);
    };
    fetchImages();
  }, []);
    ...
    <div className='h-screen w-screen overflow-x-hidden bg-gray-100 dark:bg-gray-900 font-Playfair'>
      ...
      <div className='h-[80vh] w-full bg-center bg-cover' style={{ backgroundImage: `url(${BackImg})` }}>
        ...
        <main className='flex flex-col items-center justify-center h-full px-4 text-center'>
          <h1 className='font-Tapestary text-[30px] md:text-[50px] text-neutral-800 drop-shadow-lg animate-fade-in'>Truth in Every Stroke</h1>
<h5 className='font-Carattere text-[20px] md:text-[28px] text-gray-600 mt-4 drop-shadow-md animate-fade-in delay-200'>
  Witness the world through a lens of accuracy and honesty, where art mirrors reality in breathtaking detail.
</h5>
  ...
  <h2 className='text-2xl md:text-3xl font-bold font-Quicksand text-gray-800 dark:text-gray-200 mb-4'>Uncover the Mystery of Surrealism</h2>
  <p className='text-base md:text-lg text-gray-600 dark:text-gray-300'>
    Surrealist art bends reality into dreams and symbols. Let your mind wander through fantastical landscapes and twisted logic.
  </p>
</div>
      ...
      <AnimatePresence>
        {lightbox.open && (
          ...
          >
            ...
            >
              <img
                src={surrealismImages[lightbox.index].url}
                alt={surrealismImages[lightbox.index].title || 'Landscape image'}
              ...
              <div className="absolute bottom-4 left-0 right-0 text-center text-white font-Quicksand">
                <p>{surrealismImages[lightbox.index].title || 'Untitled'}</p>
                <p className="text-sm">{lightbox.index + 1} / {surrealismImages.length}</p>
               <div className="absolute top-4 left-4">
                    <ArtworkViewTracker artworkId={surrealismImages[lightbox.index].$id} />
                  </div>
              </div>
            </motion.div>
          </motion.div>

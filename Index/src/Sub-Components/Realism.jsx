...
import BackImg from './Sub_components_images/realism.jpg';
import { Link } from 'react-router-dom';
import { FaHome, FaUser, FaInfoCircle, FaPalette, FaSearch, FaArrowLeft, FaArrowRight,FaRegEye, FaRegComment} from 'react-icons/fa';
...
function Realism() {
  ...
  const [realismImages, setRealismImages] = useState([]);
  const [loading, setLoading] = useState(true);
  const [error, setError] = useState(null)
  const [profileImage, setProfileImage] = useState(null);
  const cards = infoCardsData.realism;
  ...
  useEffect(() => {
    const fetchRealismImages = async () => {
      try {
        setLoading(true);
        ...
        const response = await databases.listDocuments(
          ...
          [      
            Query.equal( 'tag', 'Realism')] // Query for landscape tag
        );
    ...
    //Get URLs for each file
      ...
      setRealismImages(imagesWithUrls.filter(image => image !== null));
      } catch (err) {
  ...
  const downloadImage = (url, title) => {
    ...
    link.href = url;
    link.download = title || 'Realism-image';
    link.click();
  };
  ...
  useEffect(() => {
    const fetchImages = async () => {
      ...
      setFilteredImages(fetchRealismImages);
    };
    fetchImages();
  }, []);
...
<h5 className='font-Carattere text-[20px] md:text-[28px] text-blue-500 mt-4 drop-shadow-md animate-fade-in delay-200'>
  ...
  <h2 className='text-2xl md:text-3xl font-bold font-Quicksand text-gray-800 dark:text-gray-200 mb-4'>Discover the Truth of Realism</h2>
  <p className='text-base md:text-lg text-gray-600 dark:text-gray-300'>
    Realist art mirrors life with precision and honesty. See the world through the artist’s lens with unmatched detail.
  </p>
</div>
      ...
      <AnimatePresence>
        {lightbox.open && (
          ...
          >
            ...
            >
              <img
                src={realismImages[lightbox.index].url}
                alt={realismImages[lightbox.index].title || 'Landscape image'}

  ...
  useEffect(() => {
    const fetchStillLifeImages = async () => {
      try {
        ...
        setStillLifeImages(validImages);
                   ...
                   >
                     <div className="flex flex-col items-center py-4 gap-y-3 font-Playfair font-bold text-black dark:text-gray-100">
                       ...
                       <Link to='/Gallery' onClick={() => { setActiveButton('Gallery'); toggleMenu(); }}>
                         <button className={`w-full py-2 px-4 flex items-center justify-center gap-2 bg-blue-500 text-white hover:bg-blue-600 ${activeButton === 'Gallery' ? 'bg-blue-600' : ''}`}>
                           <FaPalette />
                           Gallery
                         </button>
                       </Link>

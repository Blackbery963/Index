...
import ArtFeed from './components/ArtFeed'; // New Component
import DetailModal from './components/DetailModal';
import CartDrawer from './components/CartDrawer';
import Footer from './components/Footer';

// Artificial Data Generator for Demo
const generatePromos = () => [
  {
    id: 'promo-1',
    type: 'mini-collection',
    title: 'Ceramic Vases',
    subtitle: 'Handmade Earth Tones',
    items: [
      { img: 'https://images.pexels.com/photos/4207788/pexels-photo-4207788.jpeg?auto=compress&cs=tinysrgb&w=600', price: 1200 },
      { img: 'https://images.pexels.com/photos/4207828/pexels-photo-4207828.jpeg?auto=compress&cs=tinysrgb&w=600', price: 1500 },
      { img: 'https://images.pexels.com/photos/6707628/pexels-photo-6707628.jpeg?auto=compress&cs=tinysrgb&w=600', price: 900 },
      { img: 'https://images.pexels.com/photos/4207791/pexels-photo-4207791.jpeg?auto=compress&cs=tinysrgb&w=600', price: 2100 }
    ]
  },
  {
    id: 'promo-2',
    ...
    subtitle: 'The Monochrome Collection',
    image: 'https://images.pexels.com/photos/1585325/pexels-photo-1585325.jpeg?auto=compress&cs=tinysrgb&w=600',
    cta: 'Explore Series'
  },
  {
    id: 'promo-3',
    ...
    title: 'Wearable Art',
    items: [
      { img: 'https://images.pexels.com/photos/9558577/pexels-photo-9558577.jpeg?auto=compress&cs=tinysrgb&w=600', title: 'Painted Tote', price: 450 },
      { img: 'https://images.pexels.com/photos/5935246/pexels-photo-5935246.jpeg?auto=compress&cs=tinysrgb&w=600', title: 'Custom Jacket', price: 3500 },
      { img: 'https://images.pexels.com/photos/9969248/pexels-photo-9969248.jpeg?auto=compress&cs=tinysrgb&w=600', title: 'Art Scarf', price: 899 }
    ]
  },
  {
    id: 'promo-artist-1',
    ...
    name: 'Elena Vance',
    role: 'Abstract Minimalist',
    avatar: 'https://images.pexels.com/photos/3777943/pexels-photo-3777943.jpeg?auto=compress&cs=tinysrgb&w=200',
    workSample: 'https://images.pexels.com/photos/3095625/pexels-photo-3095625.jpeg?auto=compress&cs=tinysrgb&w=400',
    bio: 'Creating silence through chaos in downtown Mumbai.'
  },
  {
    id: 'promo-video-1',
    type: 'video-teaser',
    videoUrl: 'https://videos.pexels.com/video-files/3052066/3052066-hd_1080_1920_30fps.mp4', // Pexels Art Video
    title: 'Process: The Clay Series',
    duration: '0:45'
  },
  {
    id: 'promo-quote-1',
...
const ArtStore = () => {
  const [realArt, setRealArt] = useState([]);
  const [loading, setLoading] = useState(true);
  ...
  useEffect(() => {
    const loadArtwork = async () => {
      try {
        setLoading(true);
        const data = await ArtworkService.fetchArtwork();
        setRealArt(data);
      } catch (err) {
        console.error(err);
      } finally {
  ...
  // The Algorithm: Mix Real Data with Artificial Promos
  // const mixedFeed = useMemo(() => {
  //   if (loading) return [];
    
  //   // Filter real art first
  //   let filtered = realArt.filter(art => activeCategory === 'All' || art.category === activeCategory);
    
  //   // If we are in 'All', inject promos randomly
  //   if (activeCategory === 'All') {
  //     const promos = generatePromos();
  //     const feed = [...filtered];
      
  //     // Inject at random indexes (e.g., index 4, 9, 14)
  //     feed.splice(4, 0, promos[0]); // Mini Collection
  //     feed.splice(8, 0, promos[1]); // Dark Promo
  //     feed.splice(12, 0, promos[2]); // Carousel
  //     feed.splice(2, 0, promos[0]); // Artist Spotlight (Early)
  //     feed.splice(5, 0, promos[1]); // Video (Middle)
  //     feed.splice(11, 0, promos[2]); // Quote (Later)
      
  //     return feed;
  //   }
    
  //   return filtered;
  // }, [realArt, loading, activeCategory]);
  // ... inside ArtStore component
  ...
  const mixedFeed = useMemo(() => {
    ...
    if (activeCategory === 'All') {
      ...
      // NOTE: We must splice carefully. Splice shifts the array.
      // It is safer to splice from highest index to lowest, 
      // or just be careful with the order.
      
      // 1. Artist Spotlight (Index 3 in promos array) -> Insert at position 2
      feed.splice(2, 0, promos[3]); 
      
      // 2. Mini Collection (Index 0 in promos array) -> Insert at position 5
      feed.splice(5, 0, promos[0]); 
      
      // 3. Video Teaser (Index 4 in promos array) -> Insert at position 9
      feed.splice(9, 0, promos[4]); 
      
      // 4. Carousel (Index 2 in promos array) -> Insert at position 12
      feed.splice(12, 0, promos[2]); 
      
      // 5. Editorial Quote (Index 5 in promos array) -> Insert at position 15
      feed.splice(15, 0, promos[5]);

      // 6. Dark Promo (Index 1 in promos array) -> Insert at position 18
      feed.splice(18, 0, promos[1]);
...
// ... rest of component
  ...
  const addToCart = (art) => CartService.addToCart(art, cartItems, setCartItems);
  const removeFromCart = (id) => CartService.removeFromCart(id, cartItems, setCartItems);
  ...
  return (
    <div className="min-h-screen bg-zinc-50 dark:bg-zinc-950 text-zinc-900 dark:text-zinc-100 font-sans transition-colors duration-300">
      ...
      <main className="pt-24 pb-20 px-0 md:px-8 max-w-[1600px] mx-auto">
        <Hero />
      ...
      <AnimatePresence>
        {selectedArt && (
          <DetailModal art={selectedArt} onClose={() => setSelectedArt(null)} addToCart={addToCart} />
        )}
      </AnimatePresence>

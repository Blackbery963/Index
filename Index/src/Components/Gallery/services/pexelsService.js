...
//   // Only use proxy if needed (mainly for localhost)
//   return isDevelopment ? CORS_PROXIES[currentProxyIndex] : '';
// };
...
// // Enhanced axios request with CORS handling
...
//       // Remove authorization header when using proxy
//       if (proxy && config.headers.Authorization) {
//         // For proxied requests, we might need to handle auth differently
//         // Some proxies don't forward auth headers properly
//         delete config.headers.Authorization;
        
//         // Add API key as query parameter instead (for APIs that support it)
//         if (url.includes('pixabay.com')) {
//           // Pixabay already has key in URL, no change needed
//         }
//       }
...
//       // If CORS error and we have more retries, switch proxy
...
//     const totalResults = data.total_results;
//     const totalPages = Math.min(Math.ceil(totalResults / perPage), 80);
//     const hasMore = page < totalPages && media.length > 0;
...
//   try {
//     if (PEXELS_API_KEY) {
//       await makeRequest('https://api.pexels.com/v1/curated?per_page=1', {
//         headers: { Authorization: PEXELS_API_KEY }
//       });
//       status.pexels = true;
//     }
...
//   try {
//     if (PIXABAY_API_KEY) {
//       await makeRequest(`https://pixabay.com/api/?key=${PIXABAY_API_KEY}&q=test&per_page=1`);
//       status.pixabay = true;
//     }
//   } catch (error) {
//     console.warn('Pixabay API not available');
...
//   try {
//     if (UNSPLASH_API_KEY) {
//       await makeRequest('https://api.unsplash.com/photos/random?count=1', {
//         headers: { Authorization: `Client-ID ${UNSPLASH_API_KEY}` }
//       });
//       status.unsplash = true;
//     }

...
import BackImg from './Sub_components_images/minimilism.jpg';
...
function Minimalism() {
  ...
  const [minimalismImages, setMinimalismImages] = useState([]);
  const [loading, setLoading] = useState(true);
  const [error, setError] = useState(null)
  const [profileImage, setProfileImage] = useState(null);
  const cards = infoCardsData.minimalism;
  ...
  // useEffect(() => {
  //   const fetchMinimalismImages = async () => {
  //     try {
  //       setLoading(true);
  ...
  //         [      
  //           Query.equal( 'tag', 'Minimalism')] // Query for landscape tag
  //       );
  ...
  //     setMinimalismImages(imagesWithUrls.filter(image => image !== null));
  //     } catch (err) {
  ...
  //   fetchMinimalismImages();
  // }, []);
  

  useEffect(() => {
    const fetchMinimilismImages = async () => {
      try {
        setLoading(true);
        ...
        setMinimalismImages(validImages);
        ...
        setError(err.message || 'Failed to load Minimilism images');
      } finally {
        setLoading(false);
      }
    };
  
    fetchMinimilismImages();
  }, []);
  ...
  const downloadImage = (url, title) => {
    ...
    link.href = url;
    link.download = title || 'Minimalism-image';
    link.click();
  };
  ...
  const prevImage = () => setLightbox((prev) => ({
    ...prev,
    index: prev.index > 0 ? prev.index - 1 : minimalismImages.length - 1,
  }));
  const nextImage = () => setLightbox((prev) => ({
    ...prev,
    index: prev.index < minimalismImages.length - 1 ? prev.index + 1 : 0,
  }));
  ...
  useEffect(() => {
    const fetchImages = async () => {
      ...
      setFilteredImages(fetchMinimalismImages);
    };
    fetchImages();
  }, []);
    ...
    <div className='h-screen w-screen overflow-x-hidden bg-gray-100 dark:bg-gray-900 font-Playfair'>
      ...
      <div className='h-[80vh] w-full bg-center bg-cover' style={{ backgroundImage: `url(${BackImg})` }}>
        ...
        <main className='flex flex-col items-center justify-center h-full px-4 text-center'>
          <h1 className='font-Tapestary text-[30px] md:text-[50px] text-gray-700 drop-shadow-lg animate-fade-in'>Less is Infinite</h1>
<h5 className='font-Carattere text-[20px] md:text-[28px] text-gray-400 mt-4 drop-shadow-md animate-fade-in delay-200'>
  Embrace the beauty of simplicity—where each line, shape, and space speaks volumes in quiet elegance.
</h5>
  ...
  <h2 className='text-2xl md:text-3xl font-bold font-Quicksand text-gray-800 dark:text-gray-200 mb-4'>Embrace the Essence of Minimalism</h2>
  <p className='text-base md:text-lg text-gray-600 dark:text-gray-300'>
    Find clarity in simplicity. Minimalist art speaks volumes through subtlety, space, and elegant restraint.
  </p>
</div>
      ...
      <AnimatePresence>
        {lightbox.open && (
          ...
          >
            ...
            >
              <img
                src={minimalismImages[lightbox.index].url}
                alt={minimalismImages[lightbox.index].title || 'Landscape image'}

...
import BackImg from './Sub_components_images/digital.jpg';
import { Link } from 'react-router-dom';
import { FaHome, FaUser, FaInfoCircle, FaPalette, FaSearch, FaArrowLeft, FaArrowRight, FaRegEye} from 'react-icons/fa';
...
useEffect(() => {
  ...
  const downloadImage = (url, title) => {
    ...
    link.href = url;
    link.download = title || 'Digital-image';
    link.click();
  };
  ...
  const prevImage = () => setLightbox((prev) => ({
    ...prev,
    index: prev.index > 0 ? prev.index - 1 : digitalImages.length - 1,
  }));
  const nextImage = () => setLightbox((prev) => ({
    ...prev,
    index: prev.index < digitalImages.length - 1 ? prev.index + 1 : 0,
  }));
  ...
  useEffect(() => {
    const fetchImages = async () => {
      ...
      setFilteredImages(fetchDigitalImages);
    };
    fetchImages();
  }, []);
...
<h1 className='font-Tapestary text-[30px] md:text-[50px] text-blue-600 drop-shadow-lg animate-fade-in'>Pixels & Possibilities</h1>
<h5 className='font-Carattere text-[20px] md:text-[28px] text-cyan-300 mt-4 drop-shadow-md animate-fade-in delay-200'>
  Experience the fusion of technology and imagination through digital masterpieces crafted with precision and vision.
</h5>
  ...
  <h2 className='text-2xl md:text-3xl font-bold font-Quicksand text-gray-800 dark:text-gray-200 mb-4'>Dive into the World of Digital Art</h2>
  <p className='text-base md:text-lg text-gray-600 dark:text-gray-300'>
    Witness creativity without limits. Digital art fuses imagination and technology to create vivid, boundary-pushing masterpieces.
  </p>
</div>
      ...
      <AnimatePresence>
        {lightbox.open && (
          ...
          >
            ...
            >
              <img
                src={digitalImages[lightbox.index].url}
                alt={digitalImages[lightbox.index].title || 'Landscape image'}
              ...
              <div className="absolute bottom-4 left-0 right-0 text-center text-white font-Quicksand">
                <p>{digitalImages[lightbox.index].title || 'Untitled'}</p>

...
// import { Camera, Edit2, Grid3X3, SquarePen, Upload, X } from 'lucide-react';
...
//       {/* Cover Wrapper */}
//       <motion.div
//         className="w-full pt-6 md:w-[80%] mx-auto h-72 sm:h-80 md:h-96 relative rounded-2xl overflow-hidden"
//         initial={{ opacity: 0 }}
//         animate={{ opacity: 1 }}
//       >
...
//                   <button
//                     onClick={() => setShowSelector(true)}
//                     className="absolute bottom-4 right-4 z-20 bg-white/90 dark:bg-gray-800/90 p-2 rounded-full shadow-sm 0 hover:scale-105 transition-all cursor-pointer backdrop-blur-sm"
//                     type="button"
//                     aria-label="Change cover image"
//                   >
//                     <SquarePen size={12} className="text-gray-600 dark:text-gray-400" />
//                   </button>
//                 )}
//               </>
//             ) : (
...
import { predefinedCovers } from '../utils/predefinedCovers'; // Keep your import
...
const CoverSection = ({ coverImage, showCoverButton, isOwnProfile, handleCoverImage }) => {
  ...
  return (
    <>
      <div className="w-full max-w-7xl mx-auto px-1 md:px-4 pt-2 md:pt-4">
        <motion.div
          className="relative w-full aspect-[3/1] md:aspect-[4/1] min-h-[200px] rounded-lg md:rounded-3xl overflow-hidden bg-zinc-100 dark:bg-zinc-900 group"
          initial={{ opacity: 0 }}
          animate={{ opacity: 1 }}
        >
          {/* Cover Image */}
          {coverImage ? (
            ...
            />
             ...
             // Empty State Pattern
            <div className="w-full h-full opacity-10 bg-[radial-gradient(#444cf7_1px,transparent_1px)] [background-size:16px_16px]"></div>
          )}

          {/* Overlay Gradient (Better text readability if you add text later) */}
          <div className="absolute inset-0 bg-gradient-to-t from-black/20 to-transparent pointer-events-none" />

          {/* Edit Button - Only shows on hover/mobile */}
          {isOwnProfile && (
            <button
              onClick={() => setShowSelector(true)}
              className="absolute top-4 right-4 md:opacity-0 group-hover:opacity-100 transition-opacity duration-200
                         bg-black/50 hover:bg-black/70 backdrop-blur-md text-white px-3 py-1.5 
                         rounded-full text-xs font-medium flex items-center gap-2"
            >
              <Camera size={14} />
      ...
      <input
        ...
        className="hidden"
        accept="image/*"
        onChange={(e) => { handleCoverImage(e); setShowSelector(false); }}
      />
          ...
          >
            ...
            >
              <div className="flex justify-between items-center p-4 border-b border-zinc-100 dark:border-zinc-800">
                <h3 className="font-bold text-zinc-900 dark:text-white">Update Cover Photo</h3>
                <button onClick={() => setShowSelector(false)}><X size={20} className="text-zinc-500" /></button>
              </div>

              <div className="p-4 grid grid-cols-2 md:grid-cols-3 gap-3 max-h-[60vh] overflow-y-auto">
                {predefinedCovers.map((img, i) => (
                  <button key={i} onClick={() => { handleCoverImage(img); setShowSelector(false); }} className="relative aspect-video rounded-xl overflow-hidden group">
              ...
              <div className="p-4 border-t border-zinc-100 dark:border-zinc-800 bg-zinc-50 dark:bg-zinc-900/50">
                <label htmlFor="cover-upload" className="flex items-center justify-center gap-2 w-full py-3 bg-zinc-900 dark:bg-white text-white dark:text-black rounded-xl font-semibold cursor-pointer hover:opacity-90 transition-opacity">

...
function Traditional() {
  ...
  const [traditionalImages, setTraditionalImages] = useState([]);
  ...
  loadProfile();
    ...
    useEffect(() => {
    const fetchTraditionalImages = async () => {
      try {
        setLoading(true);
        ...
        setTraditionalImages(validImages);
  ...
  useEffect(() => {
    const fetchImages = async () => {
      ...
      setFilteredImages(fetchTraditionalImages);
    };
    fetchImages();
  }, []);
...
<h5 className='font-Carattere text-[20px] md:text-[28px] text-gray-100 mt-4 drop-shadow-md animate-fade-in delay-200'>
  Walk through timeless traditions and techniques that shaped centuries of artistic expression and cultural identity.
      ...
      {/* Content Section */}
      <section ref={contentRef} className='lg:w-[85%] max-w-[95%] py-12 bg-gray-100 dark:bg-gray-900  mx-auto'>
       <div className='lg:max-w-7xl  mx-auto text-center mb-8'>
      ...
      <AnimatePresence>
        {lightbox.open && (
          ...
          >
            ...
            >
              <img
                src={traditionalImages[lightbox.index].url}
                alt={traditionalImages[lightbox.index].title || 'Landscape image'}
              ...
              <div className="absolute bottom-4 left-0 right-0 text-center text-white font-Quicksand">
                ...
                <p className="text-sm">{lightbox.index + 1} / {traditionalImages.length}</p>
                  <div className="absolute top-4 left-4">
                    <ArtworkViewTracker artworkId={traditionalImages[lightbox.index].$id} />
                  </div>
              </div>
            </motion.div>
          </motion.div>

...
const PROFILE_BUCKET_ID = import.meta.env.VITE_APPWRITE_PROFILE_BUCKET_ID;
const COVER_BUCKET_ID = import.meta.env.VITE_APPWRITE_COVER_BUCKET_ID;
const DATABASE_ID = import.meta.env.VITE_APPWRITE_DATABASE_ID;
const USER_COLLECTION_ID = import.meta.env.VITE_APPWRITE_USERS_COLLECTION_ID;

// ------------------
// 1️⃣ HANDLE USER UPLOADED IMAGES
// ------------------
export const uploadImage = async (file, bucketId) => {
  try {
    if (!file.type.startsWith('image/')) {
    ...
    const fileUrl = 
      `https://nyc.cloud.appwrite.io/v1/storage/buckets/${bucketId}/files/${uploadedFile.$id}/preview?project=${import.meta.env.VITE_APPWRITE_PROJECT_ID}`;
...
// ------------------
// 2️⃣ USE PREDEFINED EXTERNAL IMAGES
// ------------------
export const useExternalImage = (url) => {
  if (!url.startsWith("http")) {
    throw new Error("Invalid image URL");
  }
  return url; // just return URL, no need to upload
};
